...
import { Config, DEFAULT_USER_AGENT } from '../config/config.js';
...
export class BrowserManager {
  ...
  private async launchBrowser(): Promise<void> {
    ...
    // Resolve a non-HeadlessChrome UA: the default config value is used as-is,
    // but an explicit null falls back to DEFAULT_USER_AGENT. This is set at the
    // context level so it covers both the HTTP header and navigator.userAgent.
    const userAgent = this.config.browser.userAgent ?? DEFAULT_USER_AGENT;
    ...
    if (this.config.stealth) {
      await applyStealth(this.context, userAgent);
    }
